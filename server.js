// ===============================================================================
// APEX TITAN v109.0 (UNIVERSAL OVERLORD v18.1 MERGE) - ULTIMATE ENGINE
// ===============================================================================
// MERGE SYNC: v108.0 (DYNAMIC) + v18.1 (UNIVERSAL) + STOCHASTIC PROBE + AI
// ===============================================================================
...
const GLOBAL_CONFIG = {
    ...
    // v18.1 UNIVERSAL STRATEGY SETTINGS
    MIN_WHALE_VALUE: 0.5,                
    GAS_LIMIT: 1100000n,                 
    MIN_PROFIT_BUFFER: "0.005",          // v18.1 Minimum net profit gasket (ETH)
    PROBABILISTIC_FILTER: 0.1,           
    PRIORITY_BRIBE: 15n,                 
    STOCHASTIC_PROBE_RATE: 0.9997,       // v18.1 Probe Trigger Rate
...
async function fetchAIAssessment(ethPrice) {
    const systemPrompt = "Professional market analyzer. Respond ONLY in JSON.";
    const userQuery = `ETH: $${ethPrice}. Suggest if universal strikes should be aggressive (BUY) or defensive (SELL).`;
    try {
...
╔════════════════════════════════════════════════════════╗
║   ⚡ APEX TITAN v109.0 | UNIVERSAL OVERLORD v18.1     ║
║   STRATEGY: STOCHASTIC PROBE + DYNAMIC WEALTH LEVERAGE ║
╚════════════════════════════════════════════════════════╝${TXT.reset}`);
...
async function initWorker(CHAIN) {
    ...
    async function safeConnect() {
        try {
            ...
            console.log(`${TXT.green}✅ CORE ${cluster.worker.id} [${ROLE}] UNIVERSAL SYNC${TXT.reset}`);
            ...
            process.on('message', (msg) => {
                ...
                if (msg.type === 'WHALE_SIGNAL' && msg.chainId === CHAIN.chainId && !isProcessing && ROLE !== "ANALYST") {
                    isProcessing = true;
                    strike(provider, wallet, apexIface, gasOracle, currentEthPrice, CHAIN, msg.target, "WHALE_SIGNAL")
            ...
            if (DIVISION === 0 || DIVISION === 1) {
                wsProvider.on("pending", async (txHash) => {
                    ...
                    // Opportunity Trigger (Whale Move OR Stochastic Volatility Probe from v18.1)
                    const stochasticTrigger = Math.random() > GLOBAL_CONFIG.STOCHASTIC_PROBE_RATE;
                    
                    if (stochasticTrigger) {
                        process.send({ type: 'WHALE_SIGNAL', chainId: CHAIN.chainId, target: CHAIN.uniswapRouter });
                    } else {
                        const tx = await provider.getTransaction(txHash).catch(() => null);
                        if (tx && tx.to && tx.value >= parseEther(GLOBAL_CONFIG.MIN_WHALE_VALUE.toString())) {
...
async function strike(provider, wallet, iface, gasOracle, ethPrice, CHAIN, target, mode) {
    try {
        ...
        // v17.2/v18.1 DYNAMIC LOAN SCALING
        ...
        // v18.1 PRE-FLIGHT SIMULATION & COST ANALYSIS
        ...
        // v18.1 Minimised Profit Buffer Logic
        const totalCostWithBuffer = l2GasCost + l1Fee + aaveFee + parseEther(GLOBAL_CONFIG.MIN_PROFIT_BUFFER);
        ...
        if (rawProfit > (totalCostWithBuffer * safetyMultiplier) / 100n) {
            const netProfit = rawProfit - (totalCostWithBuffer - parseEther(GLOBAL_CONFIG.MIN_PROFIT_BUFFER));

            console.log(`\n${TXT.green}💎 UNIVERSAL STRIKE CONFIRMED [${CHAIN.name}]${TXT.reset}`);
